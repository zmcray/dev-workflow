...
0. **Chunk sweep (safety net, every run).** Before pulling anything, look in `docs/plans/` (not `archive/`) for plans that belong to the issues in this objective and have **no `## Chunks` section** yet. For each one with 3+ Implementation Units or a unit over the chunk bar, run `/to-chunks` first. This is what makes the bridge automatic: a plan written during the day gets cut into chunks the moment the night run starts, whether or not anyone remembered. Plans for issues outside the objective are left alone... list them in one line so they are visible, do not chunk them. Log "Chunk sweep: N plans chunked, M skipped (small), K outside objective."
