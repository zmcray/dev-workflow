...
- **Commits:** conventional commits with the issue ID appended, e.g. `feat: implement upload flow [MCR-123]`, so Linear auto-links. One commit per chunk, even inside a group PR. Commit on the branch and leave the working tree clean before picking up the next chunk.
